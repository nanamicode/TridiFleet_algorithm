...
for seed in args.seeds:
    ...
    row = {k: m[k] for k in ('decisions','feedback_events','observed_reward',
        'policy_expected','random_baseline','oracle_ceiling','uplift_vs_random','mean_regret')}
...
report = dict(hours_per_seed=args.hours, detection_radius_m=6, results=results,
    ...
    interpretation='Synthetic plug-in reward score on matched audiences, not an observed A/B gain or a guarantee for real cities.')
p = Path(args.output)
